function contactSubmit() {
	...
	if (question == "" || name == "" || email == "") {
		...
		$.ajax({
			'type': 'POST',
			'url': `https://script.google.com/macros/s/1MnhaM_2JacQ_UR0HSmDCktk8T9_eFh6eSLj2ApF-VLJx2tAPIE1HXKMM/exec?function=contact&name=${name}&email=${email}&questions=${question}`,
			crossDomain: true,
			contentType: false,
...
function preregisterSubmit() {
	...
	$.ajax({
		'type': 'POST',
		'url': `https://script.google.com/macros/s/1MnhaM_2JacQ_UR0HSmDCktk8T9_eFh6eSLj2ApF-VLJx2tAPIE1HXKMM/exec?function=preregister&name=${name}&email=${email}`,
		crossDomain: true,
		contentType: false,
		dataType: 'text/javascript',
		processData: false,
